...
public class Test01 extends JFrame implements ActionListener {
    // 属性
    JTextField txtResult; //构造一个新的TextField
    ...
    // 方法
    public Test01() {
        setTitle("计算器");//将此窗体的标题设置为指定的字符串。
        setSize(240, 270); //整组件的大小，使其宽度为 d.width，高度为 d.height。
        setResizable(false);//用户不能调整窗口大小
        setLocationRelativeTo(null);//设置窗口相对于指定组件的位置。
        setDefaultCloseOperation(EXIT_ON_CLOSE);//设置用户在此窗体上发起 "close" 时默认执行的操作。

        Container contentPane = this.getContentPane();
        contentPane.setLayout(new BorderLayout(1, 5));//设置此容器的布局管理器， 构造一个具有指定组件间距的边框布局。
        JPanel pnlNorth = new JPanel();
        JPanel pnlCenter = new JPanel();//创建具有双缓冲和流布局的新 JPanel。
